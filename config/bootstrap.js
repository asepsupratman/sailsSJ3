...
module.exports.bootstrap = async function (done) {
  ...
  ////create view---------------------
  var normalizedPath = require("path").join(__dirname, "../mssql-view");
  //console.log("normalizedPath 25", normalizedPath);
  let sqls = [];
  require("fs").readdirSync(normalizedPath).forEach(function (file) {
    //console.log("normalizedPath 28", file);
    sqls.push(require(normalizedPath + '/' + file));
  });
  ...
  dbData.map(db => {
    sqls.map( sql => {
      let qList = sql();
      qList.map(async q => {
        //console.log("line 35",db,q)
        await sails.getDatastore(db).sendNativeQuery(q);
      })
    })
  })
  //console.log("normalizedPath 39", normalizedPath);
