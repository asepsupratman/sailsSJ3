...
module.exports = function updateOneRecord (req, res,db) {
  ...
  //console.log("asdfa",criteria);fadasdfasdfa;
  criteria.uid_sekolah = req.token.user.uid_sekolah;
  var query = Model
  //.findOne(_.cloneDeep(criteria), _.cloneDeep(queryOptions.populates))
  .find()
  .where(criteria)
  .usingConnection(db)
  ;
  query.exec(function found(err, matchingRecords) {
    ...
    let matchingRecord = matchingRecords[0];
    if (!matchingRecord) { return res.notFound(); }



    Model
    .update(_.cloneDeep(criteria), queryOptions.valuesToSet)
